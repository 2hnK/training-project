...
export function Sidebar() {
  const popularPosts = [
    { title: 'React 19 새로운 기능 정리', views: 1234 },
    { title: 'TypeScript 5.0 마이그레이션 가이드', views: 987 },
    { title: 'Next.js 앱 라우터 완벽 가이드', views: 856 },
    { title: 'Tailwind CSS 실전 팁 모음', views: 743 },
    { title: '프론트엔드 성능 최적화 방법', views: 621 },
  ];
  ...
  const tags = ['React', 'TypeScript', 'Next.js', 'Vue', 'Node.js', 'CSS', 'JavaScript', 'UI/UX'];
      ...
      <Card className="p-5">
        ...
        <div className="space-y-3">
          {popularPosts.map((post, index) => (
            <div 
              key={index} 
              className="flex items-start gap-3 cursor-pointer hover:text-blue-600 transition-colors group"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500 mt-1">조회 {post.views}</p>
              </div>
            </div>
          ))}
        </div>
